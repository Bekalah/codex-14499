...
export function drawTree(ctx, w, h, lineColor, nodeColor, NUM) {
  ...
  const paths = [
    ...
    [3,6],[4,6],[4,7],[5,7],[6,7],[6,8],[7,8],[8,9],
    [1,4],[2,5],[3,8],[5,6],[2,3],[4,8]
  ]; // 22 paths
...
export function drawHelix(ctx, w, h, color1, color2, NUM) {
  ...
  const amplitude = h / NUM.NINE * NUM.THREE; // h/3
  ...
  // Connect strands to form lattice
  for (let x = 0; x <= w; x += step * NUM.THREE) {
    const y1 = h/2 + amplitude * Math.sin((turns * 2 * Math.PI * x) / w);
    const y2 = h/2 + amplitude * Math.sin((turns * 2 * Math.PI * x) / w + Math.PI);
    ctx.beginPath();
    ctx.moveTo(x, y1);
    ctx.lineTo(x, y2);
    ctx.stroke();
